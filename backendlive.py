...
async def transcribe_stream(m3u8_url): print(f"Yayından ses alınıyor: {m3u8_url}")
...
with tempfile.NamedTemporaryFile(suffix=".wav") as tmpfile:
    while True:
        audio_chunk = process.stdout.read(16000 * 2 * 5)
        if not audio_chunk:
            continue
...
async def main(url): print("WebSocket sunucusu başlatıldı: ws://0.0.0.0:8000") asyncio.create_task(transcribe_stream(url)) async with websockets.serve(socket_handler, "0.0.0.0", 8000): await asyncio.Future()
