...
# Modelleri yükle
whisper_model = whisper.load_model("base")  # daha iyi için: "medium" veya "large"
...
async def transcribe_stream(m3u8_url):
    print("Yayından ses alınıyor:", m3u8_url)
    ...
    with tempfile.NamedTemporaryFile(suffix=".wav") as tmpfile:
        while True:
            audio_chunk = process.stdout.read(16000 * 2 * 5)  # 5 saniye (16kHz, 16bit)
            if not audio_chunk:
                continue
