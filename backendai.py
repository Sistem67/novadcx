...
def tts_worker(): while True: text, lang = tts_queue.get() try: with tempfile.NamedTemporaryFile(delete=True, suffix=".mp3") as fp: tts = gTTS(text=text, lang=lang) tts.save(fp.name) os.system(f"mpg123 -q {fp.name}") except Exception as e: print(f"TTS Hatası: {e}") tts_queue.task_done()
...
def fix_detected_lang(text, detected_lang): common_en_words = ['the', 'is', 'you', 'are', 'hello', 'and', 'to', 'this', 'with'] en_hits = sum(word in text.lower() for word in common_en_words) if detected_lang == 'tr' and en_hits >= 2: print("Dil düzeltildi: en") return 'en' return detected_lang
...
if name == "main": import argparse parser = argparse.ArgumentParser(description="Canlı yayın altyazı ve sesli çeviri") parser.add_argument('--url', required=True, help='M3U8 yayın linki') args = parser.parse_args()
