...
# Dil eşleme tablosu (40+ dil desteği)
LANGUAGE_MAP = {
    'en': 'English', 'tr': 'Turkish', 'de': 'German', 'fr': 'French', 'es': 'Spanish',
    'it': 'Italian', 'ru': 'Russian', 'ar': 'Arabic', 'zh': 'Chinese', 'ja': 'Japanese',
    'ko': 'Korean', 'pt': 'Portuguese', 'nl': 'Dutch', 'hi': 'Hindi', 'bn': 'Bengali',
    'vi': 'Vietnamese', 'th': 'Thai', 'id': 'Indonesian', 'pl': 'Polish', 'uk': 'Ukrainian',
    'el': 'Greek', 'he': 'Hebrew', 'sv': 'Swedish', 'da': 'Danish', 'fi': 'Finnish',
    'no': 'Norwegian', 'cs': 'Czech', 'ro': 'Romanian', 'hu': 'Hungarian', 'sk': 'Slovak',
    'bg': 'Bulgarian', 'hr': 'Croatian', 'lt': 'Lithuanian', 'sl': 'Slovenian', 'lv': 'Latvian',
    'et': 'Estonian', 'ms': 'Malay', 'fa': 'Persian', 'ur': 'Urdu', 'sw': 'Swahili'
}

# Özel terimler sözlüğü (daha kapsamlı)
TERM_DICTIONARY = {
    'en': {
        'machine learning': 'makine öğrenmesi', 'neural network': 'yapay sinir ağı',
        'accuracy': 'doğruluk', 'server': 'sunucu', 'framework': 'çatı',
        'API': 'Uygulama Programlama Arayüzü', 'cloud': 'bulut', 'database': 'veritabanı',
        'algorithm': 'algoritma', 'debug': 'hata ayıklama', 'interface': 'arayüz',
        'authentication': 'kimlik doğrulama', 'encryption': 'şifreleme',
        'blockchain': 'blok zinciri', 'artificial intelligence': 'yapay zeka',
        'iot': 'nesnelerin interneti', 'big data': 'büyük veri', 'API gateway': 'API ağ geçidi',
        'microservices': 'mikro hizmetler', 'containerization': 'kapsayıcılaştırma',
        'devops': 'devops', 'cloud native': 'bulut yerli', 'kubernetes': 'kubernetes',
        'docker': 'docker', 'serverless': 'sunucusuz', 'agile': 'çevik'
    },
    'de': {
        'maschinelles lernen': 'makine öğrenmesi', 'künstliche intelligenz': 'yapay zeka',
        'server': 'server', 'cloud': 'wolke', 'datenbank': 'veritabanı',
        'algorithmus': 'algoritma', 'schnittstelle': 'arayüz'
    },
    'fr': {
        'apprentissage automatique': 'makine öğrenmesi', 'intelligence artificielle': 'yapay zeka',
        'serveur': 'sunucu', 'nuage': 'bulut', 'base de données': 'veritabanı',
        'algorithme': 'algoritma', 'interface': 'arayüz'
    },
    'es': {
        'aprendizaje automático': 'makine öğrenmesi', 'inteligencia artificial': 'yapay zeka',
        'servidor': 'sunucu', 'nube': 'bulut', 'base de datos': 'veritabanı',
        'algoritmo': 'algoritma', 'interfaz': 'arayüz'
    },
    'ru': {
        'машинное обучение': 'makine öğrenmesi', 'искусственный интеллект': 'yapay zeka',
        'сервер': 'sunucu', 'облако': 'bulut', 'база данных': 'veritabanı',
        'алгоритм': 'algoritma', 'интерфейс': 'arayüz'
    }
}
...
def is_sentence_complete(text):
    ...
    question_words = {
        ...
        'ar': {'من', 'ماذا', 'أين', 'متى', 'لماذا', 'كيف', 'أي', 'لمن', 'بمن'},
        'ja': {'誰', '何', 'どこ', 'いつ', 'なぜ', 'どうやって', 'どれ', '誰の', '誰に'},
        'zh': {'谁', '什么', '哪里', '什么时候', '为什么', '如何', '哪个', '谁的', '给谁'},
        'ko': {'누구', '무엇', '어디', '언제', '왜', '어떻게', '어느', '누구의', '누구에게'}
    }
...
def correct_turkish_grammar(text):
    ...
    corrections = {
        ...
        r' yapmak\b': ' yapmak',
        r' yapar\b': ' yapar',
        r' l[ıi]r\b': ' lır',  # -lık/-lik eki düzeltmesi
        r'[dtn]en\b': 'den'   # -den/-dan eki düzeltmesi
    }
...
async def recognize_and_translate(url):
    ...
    # Model destekli dil listesi
    SUPPORTED_LANGUAGES = [
        'en', 'tr', 'de', 'fr', 'es', 'it', 'ru', 'zh', 'ja', 'ko', 
        'pt', 'nl', 'hi', 'vi', 'th', 'id', 'pl', 'uk', 'ar'
    ]
    
    # Dinamik dil modeli yükleme
    logger.info(f"Yüklenen model: {model.model_path}")
    logger.info(f"Desteklenen diller: {', '.join(SUPPORTED_LANGUAGES)}")
    ...
    while True:
        ...
        if recognizer.AcceptWaveform(data):
            ...
            if original_text:
                ...
                if word_count % 5 == 0 or word_count < MIN_DETECTION_WORDS:
                    detected_lang, confidence = advanced_language_detection(buffer)
                    
                    # Dil geçmişini güncelle
                    language_history.append(detected_lang)
                    
                    # Yeterli güven varsa ve desteklenen bir dilse
                    if confidence > LANGUAGE_DETECTION_THRESHOLD and detected_lang in SUPPORTED_LANGUAGES:
...
async def main(url):
    ...
    logger.info(f"Kullanılan Vosk modeli: {VOSK_MODEL_PATH}")
